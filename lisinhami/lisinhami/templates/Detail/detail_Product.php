			...
			<div class="col-lg-5">
				...
				<div class="box-price">
					<div class="price-drop">
						<?php $price = $product->price + ($product->price * $product->tax) / 100;
						if ($product->category_cd == '3') { ?>
							<span><?= number_format($price, 0, '.', ',') ?> Point</span>
							<span class="issale"></span>
						<?php } elseif ($product->discount == 0) { ?>
							<span><?= number_format($price, 0, '.', ',') ?>đ</span>
							<span class="issale"></span>
						<?php } else { ?>
							<span><?= number_format(($price - $product->discount), 0, '.', ',') ?>đ</span>
							<span class="issale "><?= number_format($price, 0, '.', ',') ?>đ</span>
						<?php } ?>
					</div>
					<div class="hanc">
						<p><?= $product->made_in ?></p>
