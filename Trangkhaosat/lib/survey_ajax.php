<?
    ...
    if(isset($_POST['getInfoSurvey']))
    {    
        if(isset($_POST['id']))
        {
            $id = $_POST['id'];

            $sql = "SELECT hdr.content, dt.answer,hdr.id as id_hdr, dt.id as id_dtl, dt.sl_asw, dt.id_asw \n"
                ."FROM t_surveyhdr hdr \n" 
                ."LEFT JOIN \n"
                ."( \n"
                ."SELECT IFNULL(asw.id,'N/A') as id_asw, dt.id, dt.id_hdr, dt.answer, CASE WHEN asw.id is null then 0 else 1 END as sl_asw \n"
                ."FROM t_surveydtl dt \n"
    ...
    if(isset($_POST['createSurvey']) && isset($_POST['id_hdr']) && isset($_POST['id_dtl']))
    {
        $id_hdr = $_POST['id_hdr'];
        $id_dtl = $_POST['id_dtl'];
