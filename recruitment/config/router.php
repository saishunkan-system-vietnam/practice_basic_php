<?php

// PHP
define('FILE_PHP_SESSION_COOKIE','session_cookie.php');
define('FILE_PHP_EDIT_ACCOUNT','edit_account.php');
define('FILE_PHP_RECRUITMENT','recruitment.php');
define('FILE_PHP_MANAGE_ACCOUNT','manage_account.php');
define('FILE_PHP_HEADER','header_bar.php');
define('FILE_PHP_FOOTER','footer_bar.php');
define('FILE_PHP_SIDEBAR','sidebar.php');
define("SITE_URL","http://minhnn.com");
define('FILE_PHP_LOGOUT','logout.php');
define('FILE_PHP_SIGNUP','signup.php');
define('FILE_PHP_INDEX','index.php');
define('FILE_PHP_LOGIN','login.php');
define('FILE_PHP_ADMIN','admin.php');

// CSS
define('FILE_CSS_MANAGE_RECRUITMENT','./css/manage_recruitment.css');
define('FILE_CSS_CHANGE_PASSWORD','./css/change_password.css');
define('FILE_CSS_MANAGE_ACCOUNT','./css/manage_account.css');
define('FILE_CSS_DASHBOARD','./css/dashboard.css');
define('FILE_CSS_LISTVIEW','./css/listview.css');
define('FILE_CSS_SIDEBAR','./css/sidebar.css');
define('FILE_CSS_SIGNUP','./css/signup.css');
define('FILE_CSS_INDEX','./css/index.css');
define('FILE_CSS_LOGIN','./css/login.css');
define('FILE_CSS_APPLY','./css/apply.css');

// Javascript
define('FILE_JS_SIGNUP_RECRUITMENT','./js/signup_recruitment.js');
define('FILE_JS_MANAGE_RECRUITMENT','./js/manage_recruitment.js');
define('FILE_JS_MANAGE_ACCOUNT','./js/manage_account.js');
define('FILE_JS_CHECK_EMAIL','./js/check_email.js');
define('FILE_JS_APPLY','./js/apply.js');
define('FILE_JS_SIDEBAR','./js/sidebar.js');
define('FILE_JS_COMMON','./js/common.js');
define('FILE_JS_INDEX','./js/index.js');
define('FILE_JS_LOGIN','./js/login.js');
define('FILE_JS_SIGNUP','./js/signup.js');
?>