<?php
    ...
    if (isset($_POST["email"]) && isset($_POST["token"]) && isset($_POST["password"])) {
        ...
        $resultSelect = $mysqli->query("SELECT * FROM account WHERE id = '$email' AND token = '$token' AND del_flg = 0 AND ADDTIME(update_datetime,'0:05:00') > CURRENT_TIMESTAMP()");

        // Trường hợp là mail đổi mật khẩu và trong thời gian cho phép.
        if ($resultSelect->num_rows){
            $resultUpdate = $mysqli->query("UPDATE account SET password='$password', update_datetime = CURRENT_TIMESTAMP() WHERE id='$email' AND del_flg = 0");
