...
class Pagination {
    ...
    function html() {
        ...
        if ($this->_config['total_record'] > $this->_config['limit']) {
            $p = '<div id="pagination" class="pagination modal-5">';
 
            // Nút prev và first
            if ($this->_config['current_page'] > 1) {
                // $p .= '<a class="page-item" href="' . $this->__link('1') . '" title="1">&laquo</a>';
                $p .= '<a  class="page-item" href="' . $this->__link($this->_config['current_page'] - 1) . '" title="'.($this->_config['current_page'] - 1).'">&laquo</a>';
            }
            ...
            for ($i = $this->_config['min']; $i <= $this->_config['max']; $i++) {
                ...
                if ($this->_config['current_page'] == $i) {
                    $p .='<strong class="current-page page-item">'. $i .'</strong>';
                } else {
                    $p .= '<a class="page-item" href="' . $this->__link($i) . '" title="'.$i.'">' . $i . '</a>';
                }
            }
 
            // Nút last và next
            if ($this->_config['current_page'] < $this->_config['total_page']) {
                $p .= '<a class="page-item" href="' . $this->__link($this->_config['current_page'] + 1) . '" title="'.($this->_config['current_page'] + 1).'">&raquo;</a>';
                // $p .= '<a class="page-item" href="' . $this->__link($this->_config['total_page']) . '" title="'.$this->_config['total_page'].'">&raquo;</a>';
            }
            $p .= '</div>';
        }
        return $p;
